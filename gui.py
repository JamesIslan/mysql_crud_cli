...
def edit(window,connection,reg,new_reg):
    reg_modified = {'nome': new_reg[0].value, 'email': new_reg[1].value, 'sexo': new_reg[2].value, 'telefone': new_reg[3].value}
...
def window_add():
    ...
    button = PushButton(window,grid=[2,4],text="Cadastrar",command=add,args=[window,input_name, input_email,sex_input,phone_input])
